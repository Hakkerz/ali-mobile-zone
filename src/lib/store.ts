...
// ---------- Admin Auth ----------
export const ADMIN_PASSWORD = import.meta.env.VITE_ADMIN_PASSWORD ?? "ali2024";
export function useAdminAuth() {
  const [authed, setAuthed] = useStored<boolean>(LS.adminAuth, false);
  return {
    authed,
    login: (pw: string) => (pw === ADMIN_PASSWORD ? (setAuthed(true), true) : false),
...
export function useUserAuth() {
  ...
  const [users, setUsers] = useStored<Array<User>>("amz:user-list", []);
  ...
  const signup = (email: string, password: string): SignupResult => {
    ...
    const newUser = { email: normalized, createdAt: new Date().toISOString() };
